...
const Dashboard = props => {
  ...
  const logged = localStorage.getItem("@ulift-name");
 navigator.geolocation.getCurrentPosition((position) => {
  const user = {
    id: localStorage.getItem("@ulift"),
    name: localStorage.getItem("@ulift-name"),
    location: {
  ...
  if(socket)socket.emit("login", user, (callback) => {
    ...
    <>
      ...
      <div class="media">
        <div class="media-left">
          <img height="60px" src={Ride}class="media-object" ></img> 
        </div>
        <div class="media-body">
          <h4 class="media-heading">{logged}</h4>
          <text>Dar Carona</text> <input type="checkbox" class="form-check-input" id="driver-check"></input>
        </div>
      </div>
      </div>
      ...
      <p>Motoristas:</p>
			...
			<div class="media" key={user.id}>
			  ...
			  <div class="media-body">
				...
				<a href='#' onClick={() => request(user.id)}><img src={Logo} height='20px'></img>{`distância: ${calculateDistance(user) / 1000} km`} </a>
			  </div>
			</div>
			))}
